...
class KeyCloakAuthenticator(GenericOAuthenticator):
    ...
    async def _get_oidc_configs(self):
        ...
        while True:
            try:
                ...
                self.log.error(
                    "Failure to retrieve the openid configuration, will try again in 1 min (auth calls will fail)")
    ...
    def _decode_token(self, token, options={}):
        ...
        try:
            ...
            self.log.info(self.client_id)
            self.log.info(self.oidc_issuer)
            self.log.info(self.jwt_signing_algorithms)
            self.log.info(options)
            print(token)
            print(self.public_key)
            print(self.client_id)
            print(self.oidc_issuer)
            print(self.jwt_signing_algorithms)
            print(options)
    ...
    async def _exchange_tokens(self, token):
        ...
        responses = await asyncio.gather(
            *(self.fetch(req, "exchanging token", parse_json=False) for req in exchange_requests))
        ...
        for response, service_name in zip(responses, self.exchange_tokens):
            ...
            self.log.info(
                'Exchanged {} token, queue time: {} s, request time: {} s'.format(service_name, queue_t, request_t))
            metric_exchange_tornado_queue_time.labels(
                "exchange_token_{}".format(service_name.replace("-", "_"))).observe(queue_t)
            metric_exchange_tornado_request_time.labels("exchange_token_{}".format(service_name.replace("-", "_")),
                                                        response.code).observe(request_t)
    ...
    async def authenticate(self, handler, data=None):
        with metric_authenticate.time():
            ...
            self.log.info("Authentication Successful for user: %s, roles: %s, admin: %s" % (
            user['name'], user_roles, user['admin']))
        ...
        with metric_refresh_user.time():
            ...
            try:
                ...
                if diff_refresh < 0:
                    ...
                    self.log.info(
                        'Failed to refresh token as refresh token expired, took {}'.format(time.time() - start))
                    return False
                    ...
                    try:
                        ...
                        self.log.error(
                            "Failed to exchange tokens during refresh, took %s seconds" % (time.time() - start),
                            exc_info=True)

                        return False

                    self.log.info(
                        'User %s oAuth tokens refreshed, took %s seconds' % (user.name, (time.time() - start)))
                    return {
                ...
                self.log.error("Failed to refresh the oAuth tokens, took %s seconds" % (time.time() - start),
                               exc_info=True)
